...
long long divs[MAXN]; // n 的所有整除结果
int sqrtn;

void get_divs(long long n) {
	...
	assert(is_sorted(divs + 1, divs + divs[0] + 1));
	divs[0] = unique(divs + 1, divs + divs[0] + 1) - divs - 1;
}

int getid(long long x) { // divs[divs[0]] 就是 n
	return x <= sqrtn ? x : (divs[0] - divs[divs[0]] / x + 1);
}
...
void get_dp(function<int(long long)> pw, function<int(long long)> sum) {
	...
	for (int i = 1; i <= divs[0]; i++)
		dp[i] = (sum(divs[i]) + p - 1) % p;
	
	for (int i = 1; i <= prime_cnt; i++) {
		int pi = prime[i];
		
		for (int j = divs[0]; (long long)pi * pi <= divs[j]; j--) {
			int k = getid(divs[j] / pi);
