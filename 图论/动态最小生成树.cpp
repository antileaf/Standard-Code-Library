...
//一个跑得比较快的做法是对时间分治，在每层分治时找出一定在/不在MST上的边，只带着不确定边继续递归
...
for(int i=q;i;i--)e[0][a[i].id].w=a[i].u;
...
void mergeset(int x,int y){//按size合并
...
void cut(int x){//并查集撤销
	int y=x;
	do size[y=p[y]]-=size[x];while(p[y]!=y);
	p[x]=x;
}