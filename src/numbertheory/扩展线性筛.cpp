...
// f：除掉最小质因子后剩下的部分
// g：最小质因子的幂次，在 f(p^k) 比较复杂时很有用，但 f(p^k) 可以递推时就可以省略了
	...
	for (int i = 2; i <= n; i++) {
		...
		for (int j = 1; j <= prime[0] && i * prime[j] <= n; j++) {
			...
			if (i % prime[j]) { // 加入一个新的质因子，这种情况很简单
				...
				f[i * prime[j]] = f[i];
				g[i * prime[j]] = g[i] + 1;
				// 对于 f(p^k) 可以直接递推的函数，这里的判断可以改成
				// i / prime[j] % prime[j] != 0, 这样可以省下 f[] 的空间，
				// 但常数很可能会稍大一些
				
				if (f[i] == 1) // 质数的幂次，这里 $\sigma_1$ 可以递推
					sigma_one[i * prime[j]] = (sigma_one[i] + i * prime[j]) % p;
					// 对于更一般的情况，可以借助 g[] 计算 f(p^k)
