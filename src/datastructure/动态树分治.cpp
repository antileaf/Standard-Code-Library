...
void build(int x, int k, int s, int pr) { // 结点, 深度, 连通块大小, 点分树上的父亲
	...
	for (int i = 0; i < (int)G[x].size(); i++)
		if (!vis[G[x][i]])
			build(G[x][i], k + 1, size[G[x][i]], x); // 递归建树
}
...
int getcenter(int x, int s) {
	...
	while (head != tail) {
		x = q[head++];
		size[x] = 1; // 这里不需要清空, 因为以后要用的话一定会重新赋值
		son[x] = 0;
	...
	for (int i = tail - 1; i; i--) {
		...
		if (size[x] > size[son[p[x]]])
			son[p[x]] = x;
	}

	x = q[0];
	while (son[x] && size[son[x]] * 2 >= s)
		x = son[x];
...
void getdis(int x, int k, int rt) {
	...
	for (int i = tail - 1; i; i--)
		size[p[q[i]]] += size[q[i]]; // 后面递归建树要用到子问题大小
}
